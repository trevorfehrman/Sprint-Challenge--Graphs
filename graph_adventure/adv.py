...
world.printRooms()
player = Player("Trevor", world.startingRoom)
...
def explore(player, direction = None):
    #base case
    if len(my_world) == len(roomGraph):
        return
    
    #add info to graph
    here = player.currentRoom.id
    if here not in my_world:  
        my_world[here] = {exit: '?' for exit in player.currentRoom.getExits() }

    #if we're not in the first invoaction on the stack anymore
    if direction is not None:
        opposite_direction = get_opposite_direction(direction)
        previous_room = player.currentRoom.getRoomInDirection(opposite_direction)
        my_world[here][opposite_direction] = previous_room.id

    #find out if we hit a dead end, if not keep moving
    for exit in player.currentRoom.getExits():
        if my_world[here][exit] == '?':
            dead_end = False
            player.travel(exit)

            #swap variables, update graph and traversal path
            previous_room = here
            here = player.currentRoom.id
            my_world[previous_room][exit] = here
            traversalPath.append(exit)

            #recurse for as many unexplored exits exist
            explore(player, exit)
            break
        else:
            dead_end = True
    ...
    #bfs to the nearest unexplored fork
    path = []
    if dead_end == True:
        #initialize
        queue = Queue()
        ...
        while queue.size() > 0:
            #TODO Hacky - Gotta be a better way to do this
            sub_path = queue.queue.pop(random.choice([0, len(queue.queue)-1]))
            latest = sub_path[-1]

            if latest not in visited:
                visited.add(latest)
                #find the nearest vertex with an unexplored room
                if '?' in my_world[latest].values():
                    path = sub_path
                    queue = Queue()
                    break

                for neighbor in my_world[latest].values():
                    fork = sub_path.copy()
    ...
    for room in path:
        here = player.currentRoom.id
        exits = my_world[here].keys()
        for exit in exits:
            if my_world[here][exit] == room:
                player.travel(exit)
                traversalPath.append(exit)
